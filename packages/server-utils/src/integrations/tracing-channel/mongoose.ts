...
import { defineIntegration, extendIntegration, getActiveSpan } from '@sentry/core';
...
import { MONGOOSE_CONTEXT_CAPTURE_CHANNELS, mongooseModuleNames } from '../../orchestrion/config/mongoose';
...
const _mongooseChannelIntegration = (() => {
  return extendIntegration(mongooseIntegration(), {
    setup(client) {
      invokeOrchestrionInstrumentation(client, mongooseModuleNames, instrumentMongoose, []);
    },
  });
}) satisfies IntegrationFn;

// mongoose < 9.7 is covered by the orchestrion-injected channels.
function instrumentMongoose(): void {
