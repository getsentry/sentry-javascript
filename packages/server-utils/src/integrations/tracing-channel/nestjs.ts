...
/**
 * The shape orchestrion's `traceSync` transform attaches to the
 * tracing-channel context for `RouterExecutionContext.prototype.create`.
...
const _nestjsChannelIntegration = (() => {
  return {
    ...
    setupOnce() {
      ...
      // callback arg (-> one handler span per call) and `end` reassigns
      // `data.result` to replace the returned handler (-> one request_context
      // span per request); `traceSync` always returns whatever `end` leaves on
      // `ctx.result`, so no opt-in is needed.
      //
