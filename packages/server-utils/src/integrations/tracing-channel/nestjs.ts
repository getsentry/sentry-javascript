...
const _nestjsChannelIntegration = (() => {
  return {
    ...
    setupOnce() {
      ...
      // `start` wraps the callback arg (-> handler span per call) and `end`
      // reassigns `data.result` to replace the returned handler (-> request_context
      // span per request); `traceSync` always returns whatever `end` leaves there.
      //
