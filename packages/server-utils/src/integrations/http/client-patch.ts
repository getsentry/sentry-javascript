...
import { getOriginalFunction, markFunctionWrapped, type WrappedFunction } from '@sentry/core';
import { getHttpClientSubscriptions } from './client-subscriptions';
import { HTTP_ON_CLIENT_REQUEST } from './constants';
import type { HttpClientRequest, HttpExport, HttpInstrumentationOptions, HttpModuleExport } from './types';

// Vendored from `@sentry/core`'s private `utils/get-default-export.ts`.
function getDefaultExport<T>(moduleExport: T | { default: T }): T {
  return (
    (!!moduleExport &&
...
// Vendored from `@sentry/core`'s private `utils/object.ts`.
function wrapMethod<O extends {}, T extends string & keyof O>(
  obj: O,
  field: T,
