...
import {
  _INTERNAL_withRandomSafeContext,
  captureEvent,
...
export class SentrySpanExporter {
  ...
  public export(span: ReadableSpan): void {
    const currentTimestampInS = _INTERNAL_withRandomSafeContext(() => Math.floor(Date.now() / 1000));
  ...
  public flush(): void {
    ...
    const expirationDate = _INTERNAL_withRandomSafeContext(() => Date.now()) + DEFAULT_TIMEOUT * 1000;
  ...
  private _flushSentSpanCache(): void {
    const currentTimestamp = _INTERNAL_withRandomSafeContext(() => Date.now());
