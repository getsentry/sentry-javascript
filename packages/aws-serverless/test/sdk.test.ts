...
describe('AWSLambda', () => {
  ...
  describe('wrapHandler() on streaming handlers', () => {
    const STREAMING_SYMBOL = Symbol.for('aws.lambda.runtime.handler.streaming');
    ...
    test('successful execution', async () => {
      ...
      (streamingHandler as any)[STREAMING_SYMBOL] = 'response';
    ...
    test('preserves streaming symbol on wrapped handler', () => {
      ...
      expect((wrappedHandler as any)[STREAMING_SYMBOL]).toBe('response');
    });
