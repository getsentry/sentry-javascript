import { captureException, isThenable, startSpan, withIsolationScope } from '@sentry/core';
...
/**
 * Wrap a scheduled handler (`@Cron`/`@Interval`/`@Timeout`): fork the isolation
 * scope and capture errors. NOT async. Preserve the handler's sync return type,
 * so sync and async errors are handled on separate paths.
 */
 ...
 * `wrapHandler`), preserving the handler name and marking it wrapped. Shared by
 * the OTel schedule/event decorator wraps and the orchestrion factory subscriber.
 */
 ...
 * version. Shared by the OTel `@Processor` wrap and the orchestrion subscriber.
 */
