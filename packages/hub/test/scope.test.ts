...
describe('Scope', () => {
  ...
  describe('clone', () => {
    ...
    test('_requestSessionStatus clone', () => {
      const parentScope = new Scope();
      (parentScope as any)._requestSessionStatus = RequestSessionStatus.Errored;
      const scope = Scope.clone(parentScope);
      expect((parentScope as any)._requestSessionStatus).toEqual((scope as any)._requestSessionStatus);
    });
    ...
    test('child override does not set the value of parent _requestSessionStatus', () => {
      // Test that ensures if the Request Session Status is changed in a child scope
      // that it should not change in parent scope because we are only copying theit values not reference
      const parentScope = new Scope();
      (parentScope as any)._requestSessionStatus = RequestSessionStatus.Errored;
      ...
      expect((parentScope as any)._requestSessionStatus).toEqual(RequestSessionStatus.Errored);
      expect((scope as any)._requestSessionStatus).toEqual(RequestSessionStatus.Ok);
    });
  });
  ...
  describe('update', () => {
    ...
    test('given another instance of Scope, it should merge two together, with the passed scope having priority', () => {
      ...
      (localScope as any)._requestSessionStatus = RequestSessionStatus.Ok;
      ...
      expect(updatedScope._requestSessionStatus).toEqual(RequestSessionStatus.Ok);
    });
    ...
    test('given a plain object, it should merge two together, with the passed object having priority', () => {
      const localAttributes = {
        ...
        tags: { bar: '3', baz: '4' },
        user: { id: '42' },
        requestSessionStatus: RequestSessionStatus.Errored,
      };
      ...
      expect(updatedScope._level).toEqual(Severity.Warning);
      expect(updatedScope._fingerprint).toEqual(['bar']);
      expect(updatedScope._requestSessionStatus).toEqual(RequestSessionStatus.Errored);
    });
  });
