...
sentryTest(
  'should add resource spans to pageload transaction',
  async ({ getLocalTestPath, page, browser, isTracingCapableBundle }) => {
    if (!isTracingCapableBundle()) {
      sentryTest.skip();
    }

    // Intercepting asset requests to avoid network-related flakiness and random retries (on Firefox).
    await page.route('**/path/to/image.svg', (route: Route) =>
      route.fulfill({ path: `${__dirname}/assets/image.svg` }),
    );
    await page.route('**/path/to/script.js', (route: Route) =>
      route.fulfill({ path: `${__dirname}/assets/script.js` }),
    );
    await page.route('**/path/to/style.css', (route: Route) =>
