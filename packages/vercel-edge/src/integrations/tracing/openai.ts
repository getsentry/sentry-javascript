/**
 * OpenAI integration for Vercel Edge Runtime.
 * 
 * Like Cloudflare Workers, this does not use OpenTelemetry instrumentation
 * because Edge Runtime does not support it.
 * 
...
const _openAiIntegration = ((options: OpenAiOptions = {}) => {
  return {
    ...
    options,
    // No automatic setup in Edge Runtime - users must manually instrument
  };
 ...
 * @example
 * ```javascript
 * import { Sentry } from '@sentry/vercel-edge';
 * import { instrumentOpenAiClient } from '@sentry/vercel-edge';
 ...
 * ```
 *
 * Because Vercel Edge Runtime does not support OpenTelemetry auto-instrumentation,
