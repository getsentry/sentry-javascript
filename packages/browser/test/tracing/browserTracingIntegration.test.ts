...
import { URL_FULL, URL_PATH, SENTRY_SEGMENT_NAME_SOURCE } from '@sentry/conventions/attributes';

import { getActiveSpan, getCurrentScope, getDynamicSamplingContextFromSpan, getMainCarrier, SEMANTIC_ATTRIBUTE_SENTRY_OP, SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, setCurrentClient, spanIsSampled, spanToJSON, startInactiveSpan, TRACING_DEFAULTS, browserPerformanceTimeOrigin, getSpanDescendants } from '@sentry/core/browser';
...
describe('browserTracingIntegration', () => {
  ...
  it('sets source to "custom" if name is changed in beforeStartSpan', () => {
    ...
    expect(spanToJSON(pageloadSpan!).data[SENTRY_SEGMENT_NAME_SOURCE]).toBe('custom');
  });
