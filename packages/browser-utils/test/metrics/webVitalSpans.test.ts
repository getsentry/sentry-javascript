...
describe('_sendTtfbSpan', () => {
  ...
  it('sends a streamed TTFB span with duration from navigation start to first byte', () => {
    _sendTtfbSpan(300, mockClient);

    expect(SentryCore.startInactiveSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'TTFB',
        attributes: expect.objectContaining({
          'sentry.origin': 'auto.http.browser.ttfb',
          'sentry.op': 'ui.webvital.ttfb',
        }),
        startTime: 1, // timeOrigin: 1000 / 1000
      }),
    );

    expect(mockSpan.addEvent).toHaveBeenCalledWith('ttfb', {
      'sentry.measurement_unit': 'millisecond',
      'sentry.measurement_value': 300,
    });

    // endTime = timeOrigin + ttfbValue = 1 + 300/1000 = 1.3
    expect(mockSpan.end).toHaveBeenCalledWith(1.3);
  });
});

describe('_sendFcpSpan', () => {
  const mockSpan = {
    addEvent: vi.fn(),
    end: vi.fn(),
  };
  ...
  it('sends a streamed FCP span with duration from navigation start to first contentful paint', () => {
    _sendFcpSpan(1200);
    ...
    expect(SentryCore.startInactiveSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'FCP',
        attributes: expect.objectContaining({
          'sentry.origin': 'auto.http.browser.fcp',
          'sentry.op': 'ui.webvital.fcp',
        }),
        startTime: 1, // timeOrigin: 1000 / 1000
      }),
    );

    expect(mockSpan.addEvent).toHaveBeenCalledWith('fcp', {
      'sentry.measurement_unit': 'millisecond',
      'sentry.measurement_value': 1200,
    });

    // endTime = timeOrigin + fcpValue = 1 + 1200/1000 = 2.2
    expect(mockSpan.end).toHaveBeenCalledWith(2.2);
  });
});

describe('_sendFpSpan', () => {
  const mockSpan = {
    addEvent: vi.fn(),
    end: vi.fn(),
  };
  ...
  it('sends a streamed FP span with duration from navigation start to first paint', () => {
    _sendFpSpan(800);
    ...
    expect(SentryCore.startInactiveSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'FP',
        attributes: expect.objectContaining({
          'sentry.origin': 'auto.http.browser.fp',
          'sentry.op': 'ui.webvital.fp',
        }),
        startTime: 1, // timeOrigin: 1000 / 1000
      }),
    );

    expect(mockSpan.addEvent).toHaveBeenCalledWith('fp', {
      'sentry.measurement_unit': 'millisecond',
      'sentry.measurement_value': 800,
    });

    // endTime = timeOrigin + fpValue = 1 + 800/1000 = 1.8
    expect(mockSpan.end).toHaveBeenCalledWith(1.8);
  });
});
