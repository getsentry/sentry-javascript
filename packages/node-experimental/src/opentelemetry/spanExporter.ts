...
/** Send the given spans, but only if they are part of a finished transaction. Returns the unsent spans. */
function maybeSend(spans: OtelSpan[]): OtelSpan[] {
  const grouped = groupOtelSpansWithParents(spans);
  const remaining = new Set(grouped);

  const rootNodes = grouped.filter(node => node.span && !node.parentNode);

  rootNodes.forEach(root => {
    remaining.delete(root);
    const span = root.span as OtelSpan;
