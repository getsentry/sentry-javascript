 ...
 *
 * This wrapper:
 * - Captures errors from SSR HTML rendering
 * - Traces the RSC-to-HTML conversion phase
 * - Creates spans for `renderHTML` execution
 *
 * @param originalFn - The original `unstable_routeRSCServerRequest` function from react-router
 * @returns A wrapped function with Sentry instrumentation
 *
 * @example
 * ```typescript
 * // entry.ssr.ts
 ...
 *
 * export async function generateHTML(request: Request) {
 *   return sentryRouteRSCServerRequest({
 *     request,
 *     fetchServer: (req) => rscHandler.fetch(req),
