 ...
 *
 * This wrapper:
 * - Creates a transaction for server function execution
 * - Captures errors with proper mechanism tags
 * - Supports both sync and async server functions
 * - Flushes events for serverless environments
 *
 ...
 * @returns The wrapped server function with the same signature
 *
 * @example
 * ```typescript
 * // actions.ts
 ...
 * ```typescript
 * // With custom options
 * export const deleteUser = wrapServerFunction(
 *   "deleteUser",
 *   _deleteUser,
 *   {
 *     name: "Delete User Account",
 *     attributes: { "user.action": "delete" },
 *   }
 * );
 * ```
 */
