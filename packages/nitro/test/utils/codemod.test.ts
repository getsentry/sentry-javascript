...
describe('removeSentryQueryFromPath', () => {
  it('strips the Sentry query part from the path', () => {
    const url = `/example/path?sentry-query-wrapped-entry${SENTRY_WRAPPED_FUNCTIONS}foo,${QUERY_END_INDICATOR}`;
    const url2 = `/example/path?sentry-query-wrapped-entry${QUERY_END_INDICATOR}`;
...
describe('constructWrappedFunctionExportQuery', () => {
  ...
  it('logs a warning if no functions are found for re-export and debug is true', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      "[Sentry] No functions found to wrap. In case the server needs to export async functions other than `handler` or  `server`, consider adding the name(s) to Sentry's build options `sentry.experimental_entrypointWrappedFunctions`.",
    );
