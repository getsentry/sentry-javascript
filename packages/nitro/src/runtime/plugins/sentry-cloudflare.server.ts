...
/**
 * Sentry Cloudflare Nitro plugin for when using the "cloudflare-pages" preset in Nitro.
 ...
 * import { sentryCloudflareNitroPlugin } from '@sentry/nitro/plugins'
 *
...
export const sentryCloudflareNitroPlugin =
  ...
  (nitroApp: NitroApp): void => {
    ...
    nitroApp.hooks.hook('render:response', (response, { event }: { event: H3Event }) => {
      ...
      const contentType = String(event.node.res?.getHeaders()['content-type']);
      ...
      if (storedTraceData && Object.keys(storedTraceData).length > 0) {
        ...
        addSentryTracingMetaTags(response, storedTraceData);
      } else {
