    ...
    Object.assign(
      (event => {
        if (event.type === 'transaction') {
          ...
          // Filter out transactions for requests requesting public assets.
          // These transactions usually don't contain any spans.
          if (event.transaction?.match(/^GET \/.+/) && event.spans?.length === 0) {
            return null;
          }
