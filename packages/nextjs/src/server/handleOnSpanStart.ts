...
export function handleOnSpanStart(span: Span): void {
  ...
  if (spanAttributes?.[ATTR_NEXT_SPAN_TYPE] === 'Middleware.execute') {
    ...
    // Since Next.js 16, Node.js middleware/proxy runs as its own root span instead of a child of
    // `BaseServer.handleRequest`. `enhanceHandleRequestRootSpan` only sets the middleware op for
    // `BaseServer.handleRequest` roots, so we set it here for the middleware root span itself -
    // otherwise the generic OTel http naming would give it a `http` op and a `GET middleware GET` name.
    if (isRootSpan) {
