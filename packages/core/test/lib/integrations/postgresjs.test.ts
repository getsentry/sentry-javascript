  ...
  describe('_reconstructQuery', () => {
    ...
    describe('integration with _sanitizeSqlQuery', () => {
      it('preserves $n placeholders per OTEL spec', () => {
        ...
        expect(_sanitizeSqlQuery(_reconstructQuery(strings))).toBe('SELECT * FROM users WHERE id = $1 AND name = $2');
      });

      it('collapses IN clause with $n to IN ($?)', () => {
        const strings = ['SELECT * FROM users WHERE id = ', ' AND status IN (', ', ', ', ', ')'];
        expect(_sanitizeSqlQuery(_reconstructQuery(strings))).toBe(
          'SELECT * FROM users WHERE id = $1 AND status IN ($?)',
        );
      });

      it('returns Unknown SQL Query for undefined input', () => {
        expect(_sanitizeSqlQuery(_reconstructQuery(undefined))).toBe('Unknown SQL Query');
      });

      it('normalizes whitespace and removes trailing semicolon', () => {
        const strings = ['SELECT *\n  FROM users\n  WHERE id = ', ';'];
        expect(_sanitizeSqlQuery(_reconstructQuery(strings))).toBe('SELECT * FROM users WHERE id = $1');
      });
    });
  });
  ...
  describe('_sanitizeSqlQuery', () => {
    describe('passthrough (no literals)', () => {
      it.each([
        ...
        expect(_sanitizeSqlQuery(input)).toBe(expected);
      });
    });
    ...
    describe('numeric literal sanitization', () => {
      ...
      it('preserves numbers in identifiers', () => {
        expect(_sanitizeSqlQuery('SELECT * FROM users2 WHERE col1 = 5')).toBe('SELECT * FROM users2 WHERE col1 = ?');
        expect(_sanitizeSqlQuery('SELECT * FROM "table1" WHERE "col2" = 5')).toBe(
    ...
    describe('boolean literal sanitization', () => {
      ...
      it('does not affect identifiers containing TRUE/FALSE', () => {
        expect(_sanitizeSqlQuery('SELECT TRUE_FLAG FROM users WHERE active = TRUE')).toBe(
    ...
    describe('complex real-world queries', () => {
      it('handles query with comments, whitespace, and IN clause', () => {
        const input = `
          ...
          AND status IN ($2, $3, $4);
        `;
        expect(_sanitizeSqlQuery(input)).toBe('SELECT * FROM users WHERE id = $1 AND status IN ($?)');
      });
      ...
      it('handles Prisma-style query', () => {
        const input = `
          ...
          LIMIT $2;
        `;
        expect(_sanitizeSqlQuery(input)).toBe(
      ...
      it('handles INSERT/UPDATE with mixed literals and params', () => {
        expect(_sanitizeSqlQuery("INSERT INTO users (name, age, active) VALUES ('John', 30, TRUE)")).toBe(
          'INSERT INTO users (name, age, active) VALUES (?, ?, ?)',
        );
        expect(_sanitizeSqlQuery("UPDATE users SET name = $1, updated_at = '2024-01-01' WHERE id = 123")).toBe(
    ...
    describe('regression tests', () => {
      it('does not replace $n with ? (OTEL compliance)', () => {
        const result = _sanitizeSqlQuery('SELECT * FROM users WHERE id = $1');
        expect(result).not.toContain('?');
        expect(result).toBe('SELECT * FROM users WHERE id = $1');
      });

      it('does not split decimal numbers into ?.?', () => {
        const result = _sanitizeSqlQuery('SELECT * FROM t WHERE price = 19.99');
      ...
      it('does not leave minus sign when sanitizing negative numbers', () => {
        const result = _sanitizeSqlQuery('SELECT * FROM t WHERE val = -500');
      ...
      it('handles exact queries from integration tests', () => {
        ...
        expect(_sanitizeSqlQuery('SELECT * from generate_series(1,1000) as x')).toBe(
