...
type StartSpanManualCallback = (span: ReturnType<typeof makeMockSpan>, finish: () => void) => unknown;
const startSpanManualCalls: { options: unknown; cb: StartSpanManualCallback }[] = [];

vi.mock('../../../../src/tracing', () => ({
  startSpanManual(options: unknown, cb: StartSpanManualCallback) {
    const span = makeMockSpan((options as { name: string }).name);
    activeSpans.push(span);
    startSpanManualCalls.push({ options, cb });
    return cb(span, () => span.end());
  },
  withActiveSpan(_span: unknown, cb: () => unknown) {
    return cb();
  },
}));
...
describe('patchConnectModule', () => {
  ...
  it('wraps middleware added via use to create spans', () => {
    ...
    expect(startSpanManualCalls).toHaveLength(1);
    expect((startSpanManualCalls[0]!.options as { name: string }).name).toBe(ANONYMOUS_NAME);
  });
});
...
describe('patchConnectApp', () => {
  it('wraps anonymous middleware without a route', () => {
    ...
    expect(startSpanManualCalls).toHaveLength(1);
    const spanOptions = startSpanManualCalls[0]!.options as Record<string, unknown>;
  ...
  it('uses middleware.name when available for anonymous middleware', () => {
    ...
    expect(startSpanManualCalls).toHaveLength(1);
    expect((startSpanManualCalls[0]!.options as { name: string }).name).toBe('myHandler');
  });
  ...
  it('skips span creation when there is no active parent span', () => {
    ...
    expect(startSpanManualCalls).toHaveLength(0);
  ...
  it('http.route falls back to "/" for middleware without a routeName', () => {
    ...
    const attrs = (startSpanManualCalls[0]!.options as { attributes: Record<string, unknown> }).attributes;
