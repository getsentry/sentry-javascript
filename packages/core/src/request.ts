...
export function sessionToSentryRequest(session: Session | SessionAggregates, api: APIDetails): SentryRequest {
  ...
  const envelope = createEnvelope<SessionEnvelope>(envelopeHeaders, [
    [{ type } as { type: 'session' | 'sessions' }, session],
  ]);
...
export function eventToSentryRequest(event: Event, api: APIDetails): SentryRequest {
  ...
  if (useEnvelope) {
    ...
    const itemHeaders: EventEnvelopeItem[0] = {
      type: eventType,
      // TODO: Right now, sampleRate may or may not be defined (it won't be in the cases of inheritance and
      // explicitly-set sampling decisions). Are we good with that?
      sample_rates: [{ id: samplingMethod, rate: sampleRate }],
    };
    const envelope = createEnvelope<EventEnvelope>(envelopeHeaders, [[itemHeaders, req.body]]);
