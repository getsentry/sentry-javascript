...
function createSentrySpan(
  ...
  if (kind === 'server' && parentSentrySpan && isHttpServerSpan(parentSentrySpan)) {
    return new SentrySpanWrapper(name, parent, context, links, startTime, kind, parentSentrySpan, false);
  }
