...
// `ConnectRequest` is intentionally minimal so it stays compatible with
// downstream consumers compiled under TS 3.8 (which doesn't allow `symbol`
// index signatures). The route stack is stored on the request via a Symbol
// key; we cast through a symbol-indexed type here so the access is typed
// without leaking the symbol-index into the public `ConnectRequest` type.
function getLayers(req: ConnectRequest): string[] | undefined {
...
function addNewStackLayer(req: ConnectRequest): () => void {
  ...
  return () => {
    const current = getLayers(req);
    if (Array.isArray(current) && current.length === stackLength) {
      current.pop();
    }
  };
}
...
function patchMiddleware(
  ...
  function patchedMiddleware(this: unknown, ...args: unknown[]): unknown {
    ...
    // Create the span as inactive so the middleware (and anything it calls,
    // including the connect error-handler middleware) runs with the original
    // parent span — typically the http.server span — as the active span.
    // This matches the behavior of OpenTelemetry's connect instrumentation
    // and keeps captureException calls inside middleware attached to the
    // root server span instead of the per-middleware span.
