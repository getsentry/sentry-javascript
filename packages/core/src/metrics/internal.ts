 ...
 * @param client - A client. Uses the current client if not provided.
 * @param captureSerializedMetric - A function to capture the serialized metric.
 *
 * @experimental This method will experience breaking changes. This is not yet part of
 * the stable Sentry SDK API and can be changed or removed without warning.
 */
export function _INTERNAL_captureMetric(
  beforeMetric: Metric,
  currentScope = getCurrentScope(),
  captureSerializedMetric: (client: Client, metric: SerializedMetric) => void = _INTERNAL_captureSerializedMetric,
): void {
