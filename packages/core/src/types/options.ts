...
export interface ClientOptions<TO extends BaseTransportOptions = BaseTransportOptions> {
   ...
   * @default []
   *
   * @deprecated This option only has an effect in combination with `traceLifecycle: 'static'`. With span streaming
   * (`traceLifecycle: 'stream'`, the default), the SDK ignores it. Use `ignoreSpans` instead, which works with both
   ...
   *
   * @deprecated This option only has an effect in combination with `traceLifecycle: 'static'`. With span streaming
   * (`traceLifecycle: 'stream'`, the default), the SDK ignores it. Use `beforeSendSpan` instead, which works with both
   * trace lifecycles. `beforeSendTransaction` will be removed in v12.
   *
