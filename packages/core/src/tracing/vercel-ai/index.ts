...
function onVercelAiSpanStart(span: Span): void {
  ...
  // The `ai` SDK drives the underlying provider itself, so while it's active that provider must not
  // also instrument, or every call would produce two spans. On Cloudflare, `workers-ai-provider`
  // calls the Sentry-proxied `env.AI` binding from inside `doGenerate`/`doStream`.
  //
  // Registered lazily here (not at `setupOnce`) so a direct `env.AI.run` call made before any `ai`
  // SDK call still gets its own span — mirrors the LangChain integration's timing.
