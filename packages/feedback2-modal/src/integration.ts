...
export const feedback2ModalIntegration = ((options?: Record<string, unknown>) => {
  // eslint-disable-next-line deprecation/deprecation
  return new Feedback2Modal(options);
...
export class Feedback2Modal implements Integration {
  ...
  public setupOnce(): void {
    ...
    const handleShowModal = (options: Record<string, unknown>, appendChild: (child: HTMLElement) => void): void => {
      console.log('caught request to show screenshotModal', options, appendChild);

      const dialog = DOCUMENT.createElement('dialog');
      dialog.className = 'dialog';
      appendChild(dialog);

      client.emit('createdFeedbackModal', null);
    };
    ...
    client.on('createFeedbackModal', handleShowModal);
