...
interface MongoOptions {
  ...
  useMongoose?: boolean;
  /**
   * @hidden used for testing purposes
   */
  module?: { Collection: unknown };
}
...
export class Mongo implements Integration {
  ...
  private _module?: { Collection: MongoCollection };
  ...
  public constructor(options: MongoOptions = {}) {
    ...
    this._module = options.module as { Collection: MongoCollection };
  }
  ...
  public setupOnce(_: (callback: EventProcessor) => void, getCurrentHub: () => Hub): void {
    ...
    if (this._module) {
      collection = this._module.Collection;
    } else {
      const moduleName = this._useMongoose ? 'mongoose' : 'mongodb';
      try {
        const mongodbModule = require(require.resolve(moduleName, { paths: [cwd()] })) as {
