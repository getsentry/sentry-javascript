...
export const handleRequest: (options?: MiddlewareOptions) => MiddlewareResponseHandler = options => {
  ...
  return async (ctx, next) => {
    ...
    // For static (prerendered) routes, we do not want to do anything
    // as this is run at build time
...
async function handleStaticRoute(
  ...
  try {
    ...
    // We specifically inject empty meta-tags for static routes
    // as we do not want to continue any trace here
    return injectMetaTagsInResponse(originalResponse, parametrizedRoute, '');
  } catch (e) {
    sendErrorToSentry(e);
    throw e;
  }
 ...
 */
function addMetaTagToHead(htmlChunk: string, metaTagsStr: string, parametrizedRoute?: string): string {
...
function injectMetaTagsInResponse(
  originalResponse: Response,
  parametrizedRoute: string | undefined,
  metaTagsStr = getTraceMetaTags(),
): Response {
  try {
      ...
      start: async controller => {
        ...
        try {
          for await (const chunk of bodyReporter()) {
            ...
            const modifiedHtml = addMetaTagToHead(html, metaTagsStr, parametrizedRoute);
