...
describe('Logger', () => {
  ...
  it.each([
    ...
    ['warn', 'Warning', consoleWarnSpy],
    ['error', 'Error', consoleErrorSpy],
  ] as const)('.%s() should log multiple params correctly', (loggerMethod, logLevel, consoleSpy) => {
    const prefix = '[some-prefix]';
    const logger = createLogger({ prefix, silent: false, debug: true });
  ...
  it('.debug() should log multiple params correctly', () => {
    ...
    expect(consoleDebugSpy).toHaveBeenCalledWith(`[some-prefix] Debug: Hey!`, 'this', 'is', 'a test with', 5, 'params');
  });
