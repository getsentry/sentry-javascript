...
describe('addPluginOptionTagsToScope', () => {
  ...
  it('should set include tag according to number of entries (single entry)', () => {
    setTelemetryDataOnScope(normalizeUserOptions(defaultOptions), mockedScope as unknown as Scope, 'rollup');
  ...
  it.each([
    ['auto', { auto: true }],
    ['manual', { repo: '', commit: '' }],
  ])(`should set setCommits tag to %s if the setCommits option is %s`, (expectedValue, commitOptions) => {
    setTelemetryDataOnScope(
