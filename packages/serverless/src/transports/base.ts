...
export abstract class BaseTransport implements Transport {
  ...
  private disabledUntil: Date = new Date(Date.now());
  ...
  protected async _sendWithModule(httpModule: HTTPRequest, event: Event): Promise<Response> {
    if (new Date(Date.now()) < this.disabledUntil) {
      return Promise.reject(new SentryError(`Transport locked till ${this.disabledUntil} due to too many requests.`));
    }
    ...
    return this.buffer.add(
      new Promise<Response>((resolve, reject) => {
        const sentryReq = eventToSentryRequest(event, this.api);
        ...
        const req = httpModule.request(options, (res: http.IncomingMessage) => {
          ...
          if (status === Status.Success) {
            ...
            if (status === Status.RateLimit) {
              ...
              this.disabledUntil = new Date(now + parseRetryAfterHeader(now, header));
              logger.warn(`Too many requests, backing off till: ${this.disabledUntil}`);
            }
