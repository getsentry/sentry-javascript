...
describe('PromiseBuffer', () => {
  describe('add()', () => {
    test('no limit', () => {
      ...
      const p = jest.fn(() => new Promise(resolve => setTimeout(resolve)));
      void buffer.add(p);
    ...
    test('with limit', () => {
      ...
      const producer1 = jest.fn(() => {
        task1 = new Promise(resolve => setTimeout(resolve));
        return task1;
      });
      const producer2 = jest.fn(() => new Promise(resolve => setTimeout(resolve)));
  ...
  describe('drain()', () => {
    test('without timeout', async () => {
      const buffer = makePromiseBuffer();
      for (let i = 0; i < 5; i++) {
        void buffer.add(() => new Promise(resolve => setTimeout(resolve)));
      }
  ...
  test('resolved promises should not show up in buffer length', async () => {
    const buffer = makePromiseBuffer();
    const producer = () => new Promise(resolve => setTimeout(resolve));
  ...
  test('rejected promises should not show up in buffer length', async () => {
    const buffer = makePromiseBuffer();
    const producer = () => new Promise((_, reject) => setTimeout(reject));
    const task = buffer.add(producer);
    expect(buffer.$.length).toEqual(1);
    try {
      await task;
  ...
  test('resolved task should give an access to the return value', async () => {
    ...
    const producer = () => new Promise<string>(resolve => setTimeout(() => resolve('test')));
    const task = buffer.add(producer);
  ...
  test('rejected task should give an access to the return value', async () => {
    expect.assertions(1);
    const buffer = makePromiseBuffer<string>();
    const producer = () => new Promise<string>((_, reject) => setTimeout(() => reject(new Error('whoops'))));
    const task = buffer.add(producer);
    try {
      await task;
    } catch (e) {
