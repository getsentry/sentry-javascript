...
import type { MethodWrapperOptions } from '../src/durableobject-method-wrapper';
import { wrapMethodWithSentry } from '../src/durableobject-method-wrapper';
...
vi.mock('@sentry/core', async importOriginal => {
  ...
  return {
    ...
    continueTrace: vi.fn((traceData, callback) => callback()),
...
describe('wrapMethodWithSentry', () => {
  ...
  describe('basic wrapping', () => {
    it('wraps a sync method and returns its result', () => {
      ...
      const options: MethodWrapperOptions = {
        options: {},
  ...
  describe('startNewTrace option', () => {
    ...
    it('uses continueTrace when startNewTrace is true and spanName is set', async () => {
      const handler = vi.fn().mockResolvedValue('result');
      const options: MethodWrapperOptions = {
        options: {},
        context: createMockContext(),
      ...
      expect(sentryCore.continueTrace).toHaveBeenCalledWith(
        { sentryTrace: '', baggage: '' },
        expect.any(Function),
      );
    });

    it('does not use continueTrace when startNewTrace is false', async () => {
      const handler = vi.fn().mockResolvedValue('result');
      const options: MethodWrapperOptions = {
        options: {},
        context: createMockContext(),
      ...
      expect(sentryCore.continueTrace).not.toHaveBeenCalled();
    });
  });
