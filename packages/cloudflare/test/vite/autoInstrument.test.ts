  ...
  describe('agent classes', () => {
    ...
    it('wraps an Agent imported from another module and exported by specifier', async () => {
      ...
      expect(result.code).toBe(
        [
          "import * as __SENTRY__ from '@sentry/cloudflare';",
          'const __SENTRY_OPTIONS__ = (env) => { const opts = (() => undefined)(env); return { ...opts, enableRpcTracePropagation: opts?.enableRpcTracePropagation ?? true, rpcTracePropagationBindings: ["MY_AGENT", ...(opts?.rpcTracePropagationBindings ?? [])] }; };',
...
describe('instrument file auto-detection', () => {
  ...
  it('applies the detected callback to Durable Object classes too', async () => {
    ...
    expect(result.code).toContain(
      'const __SENTRY_OPTIONS__ = (env) => { const opts = (__SENTRY_OPTIONS_CALLBACK__)(env); return { ...opts, enableRpcTracePropagation: opts?.enableRpcTracePropagation ?? true, rpcTracePropagationBindings: ["MY_DO", ...(opts?.rpcTracePropagationBindings ?? [])] }; };',
    );
