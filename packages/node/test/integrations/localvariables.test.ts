...
describeIf(NODE_MAJOR >= 18)('LocalVariables', () => {
  describe('createCallbackList', () => {
    test('Should call callbacks in reverse order', () =>
    ...
    test('only calls complete once even if multiple next', () =>
      new Promise<void>(done => {
        const { add, next } = createCallbackList<number>(n => {
          expect(n).toBe(1);
          done();
    ...
    test('calls completed if added closure throws', () =>
      new Promise<void>(done => {
        const { add, next } = createCallbackList<number>(n => {
          expect(n).toBe(10);
          done();
  ...
  describe('rateLimiter', () => {
    ...
    test('calls disable if exceeded', () =>
      new Promise<void>(done => {
    ...
    test('does not call disable if not exceeded', () => {
      const increment = createRateLimiter(
        5,
        () => {
