...
interface HttpOutgoingInstrumentationConfig extends InstrumentationConfig {
  createSpans?: boolean;
  propagateTrace?: boolean;
  ignoreOutgoingRequests?: (url: string, request: RequestOptions) => boolean;
  requestHook?: (span: Span, request: ClientRequest) => void;
  responseHook?: (span: Span, response: IncomingMessage) => void;
  applyCustomAttributesOnSpan?: (span: Span, request: HttpClientRequest | ClientRequest, response: HttpIncomingMessage | IncomingMessage) => void;
}
export const instrumentHttpOutgoing = generateInstrumentOnce(
  `${INTEGRATION_NAME}.outgoing`,
  (options?: HttpOutgoingInstrumentationConfig) => new SentryHttpInstrumentation(options),
);
...
export const httpIntegration = defineIntegration((options: HttpOptions = {}) => {
  ...
  return {
    ...
    setupOnce() {
      ...
      // On older Node versions that don't support `http.client.request.created` diagnostics channel,
      // use monkey-patching via patchHttpModule/patchHttpsModule for span creation and trace propagation.
      if (useHttpOutgoingInstrumentation) {
        instrumentHttpOutgoing({
          createSpans: options.spans,
          ...
          responseHook: options.instrumentation?.responseHook as
            | ((span: Span, response: IncomingMessage) => void)
            | undefined,
          applyCustomAttributesOnSpan: options.instrumentation?.applyCustomAttributesOnSpan as
            | ((span: Span, request: HttpClientRequest, response: HttpIncomingMessage) => void)
            | undefined,
        });
      }
    },
