...
- feat(nextjs,vercel-edge,bundler-plugins)!: Drop the `vercel-` prefix from the default `environment` on Vercel and from the deploys the bundler plugins create there. Both now use the plain value of `VERCEL_TARGET_ENV` (`production`, `preview`, or a custom environment name), with `VERCEL_ENV` as fallback. See the [migration guide](./MIGRATION.md#sentrynextjs) for details.
