...
export async function ensureBundleBuildPrereqs(options: {
  ...
  // `build:transpile` has similar dependency needs (it needs types rather than npm builds). We should do something like
  ...
  // top-level `build:bundle`, package-level `build` and `build:bundle`
    ...
    for (const dependencyDir of dependencyDirs) {
      ...
      run('yarn build:transpile', { cwd: `${packagesDir}/${dependencyDir}` });
    }
