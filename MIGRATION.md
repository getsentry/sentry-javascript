...
| `gen_ai.invoke_agent`                                                    | `{operation} {agent}` (`invoke_agent weather_assistant`), `chain {chainName}` (`chain format_prompt`), or `{operation} {functionId}` (`invoke_agent weather_agent`) | `{operation} {agent}` when the agent name is known, or `{operation}` if the SDK has none (`invoke_agent`)                |
...
Likewise, filters matching `chain format_prompt` or `invoke_agent weather_agent` no longer apply to a streamed agent span (`'invoke_agent'`).
