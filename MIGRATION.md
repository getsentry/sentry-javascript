...
| `gen_ai.invoke_agent`                                                    | The LangChain chain name, prefixed with `chain` rather than the operation (`chain format_prompt`)                           | `{operation} {agent}` when the agent name is known, or `{operation}` if the SDK has none (`invoke_agent`)                |
...
A LangChain chain name is not an agent name, so it is no longer part of a streamed agent span name. It remains available on the `langchain.chain.name` attribute. LangGraph agent names and Vercel AI `functionId`s are unchanged — both are bounded values, so they stay in the name in either lifecycle.
...
Likewise, filters matching `chain format_prompt` no longer apply to a streamed LangChain agent span (`'invoke_agent'`).
