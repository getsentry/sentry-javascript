...
class MyChatAgentBase extends AIChatAgent<Env> {
  public async onRequest(_request: Request): Promise<Response> {
    ...
    // AI binding offline, so we instrument the mock binding manually and drive it through the
    // real Vercel AI SDK + `workers-ai-provider` stack, from inside an `AIChatAgent`.
