...
test('auto-instruments a stable MCP SDK v2 server (no manual wrap)', async ({ baseURL }) => {
  ...
  await test.step('initialize handshake', async () => {
    ...
    expect(initializeSegment.attributes?.['mcp.client.name']?.value).toEqual('test-client-v2-auto');
    expect(initializeSegment.attributes?.['mcp.server.name']?.value).toEqual('Echo-V2-Auto');
