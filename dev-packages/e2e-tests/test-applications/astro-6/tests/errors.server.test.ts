...
test.describe('server-side errors', () => {
  test('captures SSR error', async ({ page }) => {
    const errorEventPromise = waitForError('astro-6', errorEvent => {
      return errorEvent?.exception?.values?.[0]?.value === "Cannot read properties of undefined (reading 'x')";
    });

    const transactionEventPromise = waitForTransaction('astro-6', transactionEvent => {
  ...
  test('captures endpoint error', async ({ page }) => {
    ...
    const transactionEventApiPromise = waitForTransaction('astro-6', transactionEvent => {
      return transactionEvent.transaction === 'GET /endpoint-error/api';
    });
    const transactionEventEndpointPromise = waitForTransaction('astro-6', transactionEvent => {
