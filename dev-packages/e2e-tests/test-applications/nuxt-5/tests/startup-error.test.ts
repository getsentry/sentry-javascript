...
function spawnCrashingServer(env: Record<string, string>): Promise<number | null> {
  return new Promise((resolve, reject) => {
    const child = spawn('node', ['.output/server/index.mjs'], {
      // Session tracking needs a release; builds without git (e.g. the E2E runner's temp copy)
      // have none injected, so pin one for the crashed-session assertion.
...
test('captures error and crashed session when a nitro plugin throws during startup', async () => {
  const errorPromise = waitForError('nuxt-5', event => event.exception?.values?.[0]?.value === 'startup-crash-test');
  const sessionPromise = waitForSession('nuxt-5', session => session.status === 'crashed');
...
test('captures error when a server module throws during bundle evaluation', async () => {
  const errorPromise = waitForError('nuxt-5', event => event.exception?.values?.[0]?.value === 'eval-crash-test');
