...
test.describe('client-side errors', async () => {
    ...
    expect(error).toMatchObject({
      exception: {
        values: [
          {
            type: 'Error',
            value: 'Error thrown from nuxt-5 E2E test app',
            mechanism: {
              handled: false,
  ...
  test('page is still interactive after client error', async ({ page }) => {
    const error1Promise = waitForError('nuxt-5', async errorEvent => {
      return errorEvent?.exception?.values?.[0]?.value === 'Error thrown from nuxt-5 E2E test app';
    });
    ...
    const error2Promise = waitForError('nuxt-5', async errorEvent => {
      return errorEvent?.exception?.values?.[0]?.value === 'Another Error thrown from nuxt-5 E2E test app';
    });
    ...
    expect(error2).toMatchObject({
      exception: {
        values: [
          {
            type: 'Error',
            value: 'Another Error thrown from nuxt-5 E2E test app',
            mechanism: {
              handled: false,
              type: 'auto.function.nuxt.vue-error',
            },
