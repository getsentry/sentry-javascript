...
test.describe("performance events", () => {
  test("capture a distributed pageload trace", async ({ page }) => {
    ...
    const clientTxnEventPromise = waitForTransaction(
      "sveltekit-2",
      (txnEvent) => {
    ...
    const serverTxnEventPromise = waitForTransaction(
      "sveltekit-2",
      (txnEvent) => {
    ...
    // connected trace
    expect(clientTxnEvent.contexts?.trace?.trace_id).toBe(
      serverTxnEvent.contexts?.trace?.trace_id,
    );

    // weird but server txn is parent of client txn
    expect(clientTxnEvent.contexts?.trace?.parent_span_id).toBe(
      serverTxnEvent.contexts?.trace?.span_id,
    );
  });
  ...
  test("capture a distributed navigation trace", async ({ page }) => {
    ...
    const clientNavigationTxnEventPromise = waitForTransaction(
      "sveltekit-2",
      (txnEvent) => {
        return (
          txnEvent?.transaction === "/users" &&
  ...
  test("record client-side universal load fetch span and trace", async ({
    page,
  }) => {
    await waitForInitialPageload(page);
    ...
    const clientNavigationTxnEventPromise = waitForTransaction(
      ...
      (txnEvent) => {
        return (
          txnEvent?.transaction === "/universal-load-fetch" &&
          txnEvent.contexts?.trace?.op === "navigation"
        );
      },
    );
    ...
    const clickPromise = page
      .getByText("Route with fetch in universal load")
      .click();
    ...
    const clientFetchSpan = clientTxnEvent.spans?.find(
      (s) => s.op === "http.client",
    );
