...
test('sends a pageload root span with a parameterized URL', async ({ page }) => {
  const transactionPromise = waitForTransaction('nuxt-4-static', async transactionEvent => {
...
test('sends component tracking spans when `trackComponents` is enabled', async ({ page }) => {
  ...
  const expected = {
    data: { 'sentry.origin': 'auto.ui.vue', 'sentry.op': 'ui.mount', 'ui.component_name': 'ErrorButton' },
