...
export async function testChangingTracingMetaTagsOnISRPage(
  ...
  // === 1. Request ===
  const clientTxnEventPromise1 = waitForTransaction('nuxt-4-static', txnEvent => {
    return txnEvent.transaction === routePath;
  });

  const serverTxnEventPromise1 = waitForTransaction('nuxt-4-static', txnEvent => {
  ...
  const clientTxnEventPromise2 = waitForTransaction('nuxt-4-static', txnEvent => {
    return txnEvent.transaction === routePath;
  });

  const serverTxnEventPromise2 = waitForTransaction('nuxt-4-static', txnEvent => {
    return txnEvent.transaction?.includes(`GET ${routePath}`) ?? false;
  });
...
export async function testExcludeTracingMetaTagsOnCachedPage(
  ...
  const serverTxnEventPromise2 = Promise.race([
    waitForTransaction('nuxt-4-static', txnEvent => {
      return txnEvent.transaction?.includes(`GET ${routePath}`) ?? false;
    }),
