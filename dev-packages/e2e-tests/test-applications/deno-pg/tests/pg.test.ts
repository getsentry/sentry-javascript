...
test('pg queries emit a db span with orchestrion-channel attributes', async ({ baseURL }) => {
  ...
  const transactionPromise = waitForTransaction('deno-pg', event => {
    return (
      event?.contexts?.trace?.op === 'http.server' &&
      (event.request?.url ?? '').includes('/test-pg') &&
  ...
  expect(firstQuery!.data?.['network.peer.port']).toBe(5432);
...
test('a nested query lands on the same transaction (AsyncLocalStorage context restored)', async ({ baseURL }) => {
  ...
  // i.e. across pg's async socket-callback dispatch. Both spans appearing
  // on the SAME http.server transaction proves denoPostgresIntegration's
