...
test('captures Mastra agent spans (invoke_agent, chat, execute_tool) with inputs/outputs and conversation id', async ({
  ...
  // across envelopes until the agent/model spans have also arrived.
  ...
  // No double instrumentation. Mastra drives the Vercel AI SDK internally, so the
  // vercel-ai integration could also emit spans for the same calls. The Mastra
  // integration must be the only source of AI spans: every gen_ai span is
  ...
  // Model span. The request model is the id passed to `openrouter(...)`; the provider
  // string is asserted loosely because it depends on the AI SDK provider.
  ...
  // The exporter maps Mastra's tool `output` to `gen_ai.tool.call.result`; here the
  // tool returns `{ city, condition: 'Sunny', temperatureC: 22 }`.
