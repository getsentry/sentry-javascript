...
test('Sends correct error event', async ({ baseURL }) => {
  const errorEventPromise = waitForError('node-core-express-otel-v1-sdk-node', event => {
...
test('Errors do not leak between requests', async ({ baseURL }) => {
  ...
  const firstErrorPromise = waitForError('node-core-express-otel-v1-sdk-node', event => {
    return !event.type && event.exception?.values?.[0]?.value === 'This is an exception with id 111';
  });

  const secondErrorPromise = waitForError('node-core-express-otel-v1-sdk-node', event => {
