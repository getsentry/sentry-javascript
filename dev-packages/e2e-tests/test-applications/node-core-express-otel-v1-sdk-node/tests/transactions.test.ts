...
test('Sends an API route transaction', async ({ baseURL }) => {
  const pageloadTransactionEventPromise = waitForTransaction('node-core-express-otel-v1-sdk-node', transactionEvent => {
    return (
  ...
  const otelPromise = waitForPlainRequest('node-core-express-otel-v1-sdk-node-otel', data => {
...
test('Sends an API route transaction for an errored route', async ({ baseURL }) => {
  const transactionEventPromise = waitForTransaction('node-core-express-otel-v1-sdk-node', transactionEvent => {
    return (
      transactionEvent.contexts?.trace?.op === 'http.server' &&
