...
test('Should emit counter, distribution, and gauge metrics', async ({ baseURL }) => {
  const countPromise = waitForMetric('deno-static', metric => {
    return metric.name === 'test.deno.count';
  });

  const distributionPromise = waitForMetric('deno-static', metric => {
    return metric.name === 'test.deno.distribution';
  });

  const gaugePromise = waitForMetric('deno-static', metric => {
