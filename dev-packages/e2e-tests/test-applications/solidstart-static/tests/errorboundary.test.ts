...
test('captures an exception', async ({ page }) => {
  const errorEventPromise = waitForError('solidstart-static', errorEvent => {
    return (
      !errorEvent.type &&
...
test('captures a second exception after resetting the boundary', async ({ page }) => {
  const firstErrorEventPromise = waitForError('solidstart-static', errorEvent => {
    return (
      !errorEvent.type &&
      errorEvent.exception?.values?.[0]?.value ===
        'Error 1 thrown from Sentry ErrorBoundary in Solid Start E2E test app'
  ...
  const secondErrorEventPromise = waitForError('solidstart-static', errorEvent => {
    return (
      !errorEvent.type &&
      errorEvent.exception?.values?.[0]?.value ===
