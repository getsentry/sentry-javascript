...
//
// This is the static counterpart of `nextjs-16-trailing-slash`. That app asserts the same
// routes on streamed spans; this one keeps the transaction-based path covered.

test('should create a correctly named pageload transaction for a static route', async ({ page }) => {
  const transactionPromise = waitForTransaction('nextjs-16-trailing-slash-static', async transactionEvent => {
