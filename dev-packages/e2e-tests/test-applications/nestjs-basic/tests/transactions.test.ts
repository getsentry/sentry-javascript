...
import { collectStreamedSpansUntilSegment, waitForStreamedSpan } from '@sentry-internal/test-utils';
...
test('Sends an app_creation span', async () => {
  // The app_creation span is emitted once at startup (NestFactory.create), before any request is
  // made, so we look back through the buffered envelopes rather than waiting for a new span.
  const span = await waitForStreamedSpan(APP_NAME, span => span.is_segment && span.name === 'Create Nest App', 0);

  // A segment span also carries the scope contexts and the SDK's integration list, which vary by
  // machine, so only the app-creation attributes are pinned here.
  expect(span).toEqual({
    ...
    attributes: expect.objectContaining({
      ...
      'nestjs.type': { type: 'string', value: 'app_creation' },
      'nestjs.version': { type: 'string', value: expect.any(String) },
      'nestjs.module': { type: 'string', value: 'AppModule' },
    }),
  });
});
