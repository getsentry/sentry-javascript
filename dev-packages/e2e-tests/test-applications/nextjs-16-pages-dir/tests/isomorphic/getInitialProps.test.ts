...
test.skip('should propagate serverside `getInitialProps` trace to client', async ({ page }) => {
  const pageloadTransactionPromise = waitForTransaction('nextjs-16-pages-dir', async transactionEvent => {
    return (
  ...
  const serverTransactionPromise = waitForTransaction('nextjs-16-pages-dir', async transactionEvent => {
    return (
