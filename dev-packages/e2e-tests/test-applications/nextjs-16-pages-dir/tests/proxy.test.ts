...
test('Should create a transaction for proxy', async ({ request }) => {
  const middlewareTransactionPromise = waitForTransaction('nextjs-16-pages-dir', async transactionEvent => {
...
test('Faulty middlewares', async ({ request }) => {
  const middlewareTransactionPromise = waitForTransaction('nextjs-16-pages-dir', async transactionEvent => {
    return transactionEvent?.transaction === 'GET /api/endpoint-behind-faulty-middleware';
  });
  ...
  await test.step('should record transactions', async () => {
    ...
    expect(middlewareTransaction.contexts?.trace?.op).toBe('http.server');
    expect(middlewareTransaction.contexts?.runtime?.name).toBe('node');
    expect(middlewareTransaction.transaction_info?.source).toBe('route');
  });
  ...
  //   // When middleware throws, the error is attached to the error route transaction
  //   expect(['GET /api/endpoint-behind-faulty-middleware', 'middleware GET']).toContain(errorEvent.transaction);
  // });
});
