...
test.skip('should report a navigation transaction for pages router navigations', async ({ page }) => {
  test.skip(process.env.TEST_ENV === 'development', 'Test is flakey in dev mode');
  const navigationTransactionPromise = waitForTransaction('nextjs-16-pages-dir', async transactionEvent => {
    return (
