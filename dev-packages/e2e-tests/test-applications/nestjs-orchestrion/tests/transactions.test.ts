...
function findSpan(
  ...
  op: string,
  origin: string,
): { description?: string; op?: string; origin?: string; data?: Record<string, unknown> } | undefined {
  return (transactionEvent.spans ?? []).find(span => span.op === op && span.origin === origin);
}
...
// op + origin produced by `@Injectable`/`@Catch` instrumentation,
// per component type.
