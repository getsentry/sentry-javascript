...
test('captures a navigation transaction directly after pageload', async ({ page }) => {
  ...
  const clientNavigationTxnPromise = waitForTransaction('sveltekit-3', txnEvent => {
...
test('captures one navigation transaction per redirect', async ({ page }) => {
  ...
  const clientNavigationRedirect2TxnPromise = waitForTransaction('sveltekit-3', txnEvent => {
    return txnEvent?.contexts?.trace?.op === 'navigation' && txnEvent?.transaction === '/redirect2';
  });

  const clientNavigationRedirect3TxnPromise = waitForTransaction('sveltekit-3', txnEvent => {
