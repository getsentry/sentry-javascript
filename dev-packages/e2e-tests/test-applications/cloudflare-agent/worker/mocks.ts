 ...
 * routed through `workers-ai-provider` stream when driven by an Agent / `AIChatAgent`.
 *
 ...
 * The native `{ response }` shape is covered separately.
 */
export class MockAi {
  public async run(_model: string, inputs: Record<string, unknown>): Promise<unknown> {
