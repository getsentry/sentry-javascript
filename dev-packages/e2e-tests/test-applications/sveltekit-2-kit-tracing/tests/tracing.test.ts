...
test('capture a distributed pageload trace', async ({ page }) => {
  const traceSpansPromise = collectStreamedSpans('sveltekit-2-kit-tracing', spansOfTrace => {
...
test('capture a distributed navigation trace', async ({ page }) => {
  const clientNavigationSpanPromise = waitForStreamedSpan('sveltekit-2-kit-tracing', span => {
    return span.name === '/users' && getSpanOp(span) === 'navigation' && span.is_segment;
  });

  const serverSpanPromise = waitForStreamedSpan('sveltekit-2-kit-tracing', span => {
...
test('captures a navigation span directly after pageload', async ({ page }) => {
  const clientPageloadSpanPromise = waitForStreamedSpan('sveltekit-2-kit-tracing', span => {
    return getSpanOp(span) === 'pageload' && span.is_segment;
  });

  const navigationTraceSpansPromise = collectStreamedSpans('sveltekit-2-kit-tracing', spansOfTrace =>
...
test('captures one navigation span per redirect', async ({ page }) => {
  const collectNavigationTrace = (route: string) =>
    collectStreamedSpans('sveltekit-2-kit-tracing', spansOfTrace =>
  ...
  const expectNavigationTrace = (
    ...
    expect(navigationSpan.attributes).toMatchObject({
      ...
      'url.full': { value: expect.stringMatching(new RegExp(`^https?:\\/\\/localhost:\\d+${path}$`)), type: 'string' },
