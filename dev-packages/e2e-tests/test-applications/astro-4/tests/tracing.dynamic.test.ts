...
test.describe('tracing in dynamically rendered (ssr) routes', () => {
  test('sends server and client pageload spans with the same trace id', async ({ page }) => {
    ...
    expect(clientPageloadSpan.attributes).toMatchObject({
      ...
      'url.path': { value: '/test-ssr', type: 'string' },
      'url.full': { value: expect.stringMatching(/^https?:\/\/localhost:\d+\/test-ssr$/), type: 'string' },
    });
...
test.describe('nested SSR routes (client, server, server request)', () => {
  ...
  const isUserPageTraceComplete = (spans: SerializedStreamedSpan[]): boolean =>
    ...
    spans.some(span => getSpanOp(span) === 'http.server' && span.name === 'GET /api/user/myUsername123.json') &&
  ...
  test('sends connected server and client pageload and request spans with the same trace id', async ({ page }) => {
    ...
    const serverHTTPServerRequestSpan = spans.find(
      span => getSpanOp(span) === 'http.server' && span.name === 'GET /api/user/myUsername123.json',
    )!;
  ...
  test('sends parametrized pageload, server and API request span names', async ({ page }) => {
    ...
    expect(clientPageloadSpan.attributes).toMatchObject({
      ...
      'sentry.origin': { value: 'auto.pageload.astro', type: 'string' },
      'sentry.segment.name.source': { value: 'route', type: 'string' },
      'url.template': { value: '/user-page/[userId]', type: 'string' },
      'url.path': { value: '/user-page/myUsername123', type: 'string' },
      'url.full': {
        value: expect.stringMatching(/^https?:\/\/localhost:\d+\/user-page\/myUsername123$/),
        type: 'string',
      },
    });
  ...
  test('sends parametrized pageload and server span names for catch-all routes', async ({ page }) => {
    ...
    expect(clientPageloadSpan.attributes).toMatchObject({
      ...
      'sentry.origin': { value: 'auto.pageload.astro', type: 'string' },
      'sentry.segment.name.source': { value: 'route', type: 'string' },
      'url.template': { value: '/catchAll/[path]', type: 'string' },
      'url.path': { value: '/catchAll/hell0/whatever-do', type: 'string' },
      'url.full': {
        value: expect.stringMatching(/^https?:\/\/localhost:\d+\/catchAll\/hell0\/whatever-do$/),
        type: 'string',
      },
    });
...
test.describe('parametrized vs static paths', () => {
  test('should use static route name for static route in parametrized path', async ({ page }) => {
    ...
    expect(clientPageloadSpan.attributes).toMatchObject({
      ...
      'sentry.origin': { value: 'auto.pageload.astro', type: 'string' },
      'sentry.segment.name.source': { value: 'route', type: 'string' },
      'url.template': { value: '/user-page/settings', type: 'string' },
      'url.path': { value: '/user-page/settings', type: 'string' },
      'url.full': { value: expect.stringMatching(/^https?:\/\/localhost:\d+\/user-page\/settings$/), type: 'string' },
    });
