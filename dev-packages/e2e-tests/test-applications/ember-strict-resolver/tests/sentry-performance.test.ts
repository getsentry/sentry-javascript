...
test('handles slow loading route', async ({ page }) => {
  ...
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
      op: 'function',
      description: 'slow-loading-route',
      data: expect.objectContaining({ 'code.function.name': 'beforeModel' }),
    }),
  );
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
  ...
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
      op: 'function',
      description: 'slow-loading-route.index',
      data: expect.objectContaining({ 'code.function.name': 'model' }),
    }),
  );
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
  ...
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
      op: 'function',
      description: 'slow-loading-route',
      data: expect.objectContaining({ 'code.function.name': 'setupController' }),
    }),
  );
  expect(transaction.spans).toContainEqual(
    expect.objectContaining({
