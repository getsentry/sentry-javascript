...
test('Sends a root: true span as its own transaction in a new trace', async ({ baseURL }) => {
  const requestTransactionPromise = waitForTransaction('effect-3-node', transactionEvent => {
    return !!transactionEvent?.spans?.some(span => span.description === 'root-span-request-marker');
  });
  const detachedTransactionPromise = waitForTransaction('effect-3-node', transactionEvent => {
