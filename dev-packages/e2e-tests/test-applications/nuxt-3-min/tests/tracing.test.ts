...
test.describe('distributed tracing', () => {
  ...
  test('capture a distributed pageload trace', async ({ page }) => {
    const clientSpanPromise = waitForStreamedSpan('nuxt-3-min', span => {
      return getSpanOp(span) === 'pageload' && span.is_segment;
    });

    // Parametrization does not work in Nuxt 3.7 yet, so the server segment keeps a method-only
    // name and has to be selected via its `url.path` attribute.
    const serverSpanPromise = waitForStreamedSpan('nuxt-3-min', span => {
      return (
        span.is_segment &&
        getSpanOp(span) === 'http.server' &&
        span.attributes['url.path']?.value === `/test-param/${PARAM}`
      );
    });
    ...
    expect(serverSpan).toMatchObject({
      name: 'GET', // method-only because the URL cannot be parametrized in Nuxt 3.7
      is_segment: true,
      attributes: expect.objectContaining({
  ...
  test('capture a distributed trace from a client-side API request with parametrized routes', async ({ page }) => {
    ...
    const clientSpansPromise = collectStreamedSpans('nuxt-3-min', spans => {
      return (
    ...
    // Parametrization does not work in Nuxt 3.7 yet, so the SSR segment keeps a method-only name
    const ssrSpanPromise = waitForStreamedSpan('nuxt-3-min', span => {
      return (
        span.is_segment &&
        getSpanOp(span) === 'http.server' &&
        span.attributes['url.path']?.value === `/test-param/user/${PARAM}`
      );
    });
    const serverReqSpanPromise = waitForStreamedSpan('nuxt-3-min', span => {
