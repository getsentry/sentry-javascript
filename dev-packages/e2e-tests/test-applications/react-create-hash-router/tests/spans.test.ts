...
test('Captures a pageload span', async ({ page }) => {
  ...
  // Matched partially: every streamed span also carries the common `sentry.sdk.*`, `sentry.release`,
  // `sentry.environment` and segment attributes, and a pageload span additionally carries the web
  // vitals. LCP is streamed as its own `ui.webvital.lcp` span once the page is hidden, so it is no
  // longer part of the pageload span's attributes.
