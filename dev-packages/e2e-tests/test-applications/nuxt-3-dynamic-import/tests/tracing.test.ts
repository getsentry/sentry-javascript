...
test.describe('distributed tracing', () => {
  ...
  test('capture a distributed pageload trace', async ({ page }) => {
    const clientSpanPromise = waitForStreamedSpan('nuxt-3-dynamic-import', span => {
  ...
  test('capture a distributed trace from a client-side API request with parametrized routes', async ({ page }) => {
    ...
    const clientSpansPromise = collectStreamedSpans('nuxt-3-dynamic-import', spans => {
      return (
    ...
    const ssrSpanPromise = waitForStreamedSpan('nuxt-3-dynamic-import', span => {
      return span.is_segment && span.name.includes('GET /test-param/user');
    });
    const serverReqSpanPromise = waitForStreamedSpan('nuxt-3-dynamic-import', span => {
