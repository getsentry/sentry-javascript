...
test('stamps captured exceptions with the trace of the active OpenTelemetry span', async ({ baseURL }) => {
  const errorEventPromise = waitForError('sveltekit-2-otlp', event => {
  ...
  expect(errorEvent.transaction).toBe('GET /api/telemetry/[id]');
});

test('stamps logs with the trace of the active OpenTelemetry span', async ({ baseURL }) => {
  const logEnvelopePromise = waitForEnvelopeItem('sveltekit-2-otlp', envelope => {
    return (
      envelope[0].type === 'log' &&
...
test('stamps metrics with the trace of the active OpenTelemetry span', async ({ baseURL }) => {
  const metricPromise = waitForMetric('sveltekit-2-otlp', metric => {
...
test('sends no envelope trace header while riding along on an OpenTelemetry span', async ({ baseURL }) => {
  const envelopePromise = waitForRequest('sveltekit-2-otlp', ({ envelope }) => {
...
test('captures errors thrown in server routes on the request trace', async ({ baseURL }) => {
  const errorEventPromise = waitForError('sveltekit-2-otlp', event => {
    return event.exception?.values?.[0]?.value === 'This is a server route error with id 246';
  });

  const response = await fetch(`${baseURL}/api/server-route-error/246`);
  ...
  const requestSpan = await waitForExportedSpan(
    span => span.name === 'GET /api/server-route-error/[id]' && span.traceId === traceId,
    `the request span for trace ${traceId}`,
  );

  expect(requestSpan.parentSpanId).toBeUndefined();
  expect(errorEvent.transaction).toBe('GET /api/server-route-error/[id]');
...
test('captures errors thrown in server load functions on the request trace', async ({ baseURL }) => {
  const errorEventPromise = waitForError('sveltekit-2-otlp', event => {
    return event.exception?.values?.[0]?.value === 'This is a server load error with id 357';
  });

  const response = await fetch(`${baseURL}/server-load-error/357`);
  expect(response.status).toBe(500);
  ...
  const requestSpan = await waitForExportedSpan(
    span => span.name === 'GET /server-load-error/[id]' && span.traceId === traceId,
    `the request span for trace ${traceId}`,
  );

  expect(requestSpan.parentSpanId).toBeUndefined();
  expect(errorEvent.transaction).toBe('GET /server-load-error/[id]');
  expect(errorEvent.exception?.values?.[0]?.mechanism).toEqual({
    handled: true,
    type: 'auto.function.sveltekit.handle_error',
  });
...
test('keeps concurrent requests on separate traces', async ({ baseURL }) => {
  const errorEventPromises = ['567', '678'].map(id =>
    waitForError('sveltekit-2-otlp', event => {
