...
/**
 * Every test here loads `/sse` and produces identically named spans, and streamed spans are buffered
 * before they flush, so an earlier test's page load can still be arriving. A pageload span starts at
 * the browser's time origin, so requiring it to start after this test navigated is what picks out
 * this test's own trace.
 */
function isPageloadSpanAfter(navigatedAt: number) {
  return (span: SerializedStreamedSpan) =>
    getSpanOp(span) === 'pageload' && span.is_segment && span.start_timestamp >= navigatedAt;
}
...
test('Waits for sse streaming when creating spans', async ({ page }) => {
  ...
  const spansPromise = collectStreamedSpans(
    'react-router-6',
    spansOfTrace =>
      spansOfTrace.some(isPageloadSpanAfter(navigatedAt)) &&
      spansOfTrace.some(span => getSpanOp(span) === 'http.client.stream'),
  );
...
test('Waits for sse streaming when sse has been explicitly aborted', async ({ page }) => {
  ...
  const spansPromise = collectStreamedSpans(
    'react-router-6',
    spansOfTrace =>
      spansOfTrace.some(isPageloadSpanAfter(navigatedAt)) && spansOfTrace.some(span => span.name === 'sse fetch call'),
  );
...
test('Aborts when stream takes longer than 5s, by not updating the span duration', async ({ page }) => {
  ...
  const spansPromise = collectStreamedSpans(
    ...
    spansOfTrace =>
      spansOfTrace.some(isPageloadSpanAfter(navigatedAt)) &&
      spansOfTrace.some(span => span.attributes['url.full']?.value === 'http://localhost:8080/sse-timeout'),
  );
