...
import { conditionalTest, isOrchestrionEnabled } from '../../../../utils/index';
        ...
        await createRunner()
          ...
          .expect({
            span: container => {
              ...
              expect(streamingSpan!.attributes[GEN_AI_RESPONSE_STREAMING_ATTRIBUTE]).toEqual({
                type: 'boolean',
                value: true,
              });
              expect(streamingSpan!.attributes[GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE]).toEqual({
                type: 'integer',
                value: 12,
              });
              expect(streamingSpan!.attributes[GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE]).toEqual({
                type: 'integer',
                value: 18,
              });
              expect(streamingSpan!.attributes[GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE]).toEqual({
                type: 'integer',
                value: 30,
              });
          ...
          .expect({
            span: container => {
              const embeddingSpans = container.items.filter(
                span => span.attributes[GEN_AI_OPERATION_NAME_ATTRIBUTE]?.value === 'embeddings',
              );
