      ...
      await createRunner()
        .expect({
          span: container => {
            ...
            expect(chatSpan!.attributes[GEN_AI_RESPONSE_MODEL]?.value).toBe('llama-3.3-70b-versatile');
            ...
            expect(streamSpan!.name).toBe('chat llama-3.1-8b-instant');
      ...
      await createRunner()
        .expect({
          span: container => {
            ...
            expect(chatSpan!.attributes[GEN_AI_RESPONSE_TEXT]?.value).toBe('["Hello from Groq mock!"]');
