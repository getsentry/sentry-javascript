    ...
    if (stream) {
      ...
      const chunks = [
        ...
        },
        {
          ...
          created: 1677652300,
          model,
          choices: [{ index: 0, delta: { content: 'Hello from Groq streaming!' }, finish_reason: null }],
        },
        {
          id: 'chatcmpl-stream-123',
          object: 'chat.completion.chunk',
      ...
      res.send({
        ...
        model,
        choices: [
          { index: 0, message: { role: 'assistant', content: 'Hello from Groq mock!' }, finish_reason: 'stop' },
        ],
...
async function run() {
  ...
  await Sentry.startSpan({ op: 'function', name: 'main' }, async () => {
    const client = new Groq({
      apiKey: 'mock-api-key',
      // `groq-sdk` appends `/openai/v1/...` to the baseURL, so it stays bare here.
