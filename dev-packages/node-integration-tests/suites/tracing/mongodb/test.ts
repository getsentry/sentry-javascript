    ...
    await createRunner(__dirname, 'scenario.js')
      .expect({
        transaction: (txn: TransactionEvent) => {
          ...
          expect(txn.spans).toHaveLength(EXPECTED_SPAN_MATCHERS.length);
          expect(txn.spans).toEqual(expect.arrayContaining(EXPECTED_SPAN_MATCHERS));
        },
      })
      .start()
      .completed();
