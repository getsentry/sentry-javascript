...
// `@mistralai/mistralai` v2 is ESM-only. Under CJS the SDK loads via `require(esm)`, which the
// orchestrion transform does not reach, so no gen_ai spans are produced there — the integration only
// auto-instruments under ESM for this package, hence `{ failsOnCjs: true }` on every case below.
          ...
          .expect({
            span: container => {
              const chatSpan = container.items.find(
                s => s.attributes[GEN_AI_RESPONSE_ID]?.value === 'chatcmpl-mock123',
              );
