      ...
      await createRunner()
        .expect({
          span: container => {
            ...
            expect(fourthSpan!.name).toBe('/v1beta/models/:model\\:generateContent');
            expect(fourthSpan!.attributes['sentry.op'].value).toBe('request_handler.express');

            // [4] http.server span for the mock endpoint
            expect(fifthSpan!.name).toBe('POST /v1beta/models/gemini-1.5-flash:generateContent');
            expect(fifthSpan!.attributes['sentry.op'].value).toBe('http.server');

            // [5] outgoing POST client span
            expect(sixthSpan!.name).toBe('POST');

            // [6] google-genai generate_content span with full (non-truncated) input
            expect(seventhSpan!.name).toBe('generate_content gemini-1.5-flash');
            expect(seventhSpan!.attributes['sentry.op'].value).toBe('gen_ai.generate_content');
            expect(seventhSpan!.attributes[GEN_AI_INPUT_MESSAGES_ATTRIBUTE].value).toContain(streamingLongContent);

            // [7] main transaction span
        ...
        await createRunner()
          .expect({
            span: container => {
              ...
              // [6] google-genai generate_content with explicitly truncated input despite streaming
              expect(seventhSpan!.name).toBe('generate_content gemini-1.5-flash');
              expect(seventhSpan!.attributes['sentry.op'].value).toBe('gen_ai.generate_content');
