...
describe('http OTel double instrumentation', () => {
  afterAll(() => {
  ...
  createCjsTests(__dirname, 'scenario.mjs', 'instrument.mjs', (createRunner, test) => {
    test('registers double spans when OTel HttpInstrumentation is also active — documents known issue', async () => {
      ...
      // since instrument.mjs initialises Sentry with debug: true.
  ...
  createCjsTests(__dirname, 'scenario.mjs', 'instrument-mitigation.mjs', (createRunner, test) => {
