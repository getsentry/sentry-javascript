...
test("warns if using ESM on Node.js versions that don't support `register()`", async ({ signal }) => {
  const nodeMajorVersion = Number(process.versions.node.split('.')[0]);
  if (nodeMajorVersion >= 18) {
    return;
  }

  const runner = createRunner({ signal }, __dirname, 'server.mjs').ignore('event').start();
...
test('does not warn if using ESM on Node.js versions that support `register()`', async ({ signal }) => {
  const nodeMajorVersion = Number(process.versions.node.split('.')[0]);
  if (nodeMajorVersion < 18) {
    return;
  }
...
test('does not warn if using CJS', async ({ signal }) => {
  const runner = createRunner({ signal }, __dirname, 'server.js').ignore('event').start();
