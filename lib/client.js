...
var Client = function Client(dsn, options) {
    ...
    this._enabled = this.dsn && process.env.NODE_ENV === 'production';
...
_.getIdent =
_.get_ident = function getIdent(result) {return result.id+'$'+result.checksum};
...
_.process = function process(kwargs) {
    ...
    kwargs['checksum'] = kwargs['checksum'] || utils.constructChecksum(kwargs);
    ...
    return {'id': kwargs['event_id'], 'checksum': kwargs['checksum']};
};
...
_.captureException = function captureError(err, kwargs, cb) {
    ...
    err.raven = {
        kwargs: kwargs,
        cb: function(kw) {cb ? cb(self.process(kw)) : self.process(kw)}
    };
    traces.once("trace", parsers.parseError); // Every time Error.prepareStackTrace is called, this is triggered
...
_.patchGlobal = function patchGlobal(cb) {module.exports.patchGlobal(this, cb)};
...
module.exports.patchGlobal = function patchGlobal(client, cb) {
    ...
    process.on('uncaughtException', function(err) {
        if(cb) {  // bind event listeners only if a callback was supplied
            client.once('logged', function() {cb(true, err)});
            client.once('error', function() {cb(false, err)});
        }
