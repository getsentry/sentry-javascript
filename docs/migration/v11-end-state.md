...
`sentry.segment.name.source` is only set on segment spans. Setting it on a child span is a no-op: `setAttribute` ignores it, and a value passed in a child span's initial attributes is dropped when the span is linked to its parent. To set it defensively, use `setSegmentNameSourceIfSegment(span, source)` from `@sentry/core`, which no-ops unless `span` is the segment.
